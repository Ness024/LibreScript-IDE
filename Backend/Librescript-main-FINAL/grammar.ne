...
CondicionalSi -> %PR_SI _ %LPAREN _ Expresion _ %RPAREN _ BloqueCodigo _ ClausulasSiNoSi:? _ ClausulaSiNo:?
    {% (d) => nodo("CondicionalSi", { condicion: d[4], bloqueSi: d[8], bloquesSiNoSi: d[10] || [], bloqueSiNo: d[12] }) %}
...
BloqueCodigo -> %LBRACE _nl Sentencias _nl %RBRACE {% (d) => nodo("Bloque", { sentencias: d[2] }) %}
              # | %LBRACE _nl %RBRACE {% () => nodo("Bloque", { sentencias: [] }) %} # Removida por redundancia

BucleMientras -> %PR_MIENTRAS _ %LPAREN _ Expresion _ %RPAREN _ BloqueCodigo
    {% (d) => nodo("BucleMientras", { condicion: d[4], bloque: d[8] }) %}
...
BuclePara -> %PR_PARA _ %LPAREN _ InicializadorPara:? _ %PUNTO_Y_COMA _ CondicionPara:? _ %PUNTO_Y_COMA _ IncrementoPara:? _ %RPAREN _ BloqueCodigo
    {% (d) => nodo("BuclePara", { inicializacion: d[4], condicion: d[8], incremento: d[12], bloque: d[16] }) %}
...
DeclaracionFuncion -> %PR_FUNCION __ %IDENTIFICADOR_GRAL _ %LPAREN _ ListaParametrosOpcional _ %RPAREN _ %DOS_PUNTOS _ Tipo _ BloqueCodigo
    {% (d) => nodo("DeclaracionFuncion", { nombre: d[2], parametros: d[6], tipoRetorno: d[12], bloque: d[14] }) %} # CORRECCIÓN AQUÍ
...
# ... (sin cambios en la sección de expresiones, se asume que la precedencia está bien definida para operadores)
...
AccesoArreglo -> LiteralPrimario _ %LBRACKET _ Expresion _ %RBRACKET
    {% (d) => nodo("AccesoArreglo", { arreglo: d[0], indice: d[4] }) %}
                | LiteralPrimario _ %LBRACKET _ Expresion _ %RBRACKET _ %LBRACKET _ Expresion _ %RBRACKET # CORRECCIÓN AQUÍ
    {% (d) => nodo("AccesoArregloDoble", { matriz: d[0], indice1: d[4], indice2: d[9] }) %} # Reajustar índice d[9] si es necesario por el cambio de RPAREN a RBRACKET (d[8] %LBRACKET, d[9] _, d[10] Expresion, d[11] _, d[12] %RBRACKET) -> indice2: d[10]
    # Corregido: AccesoArregloDoble -> LiteralPrimario _ %LBRACKET _ Expresion _ %RBRACKET _ %LBRACKET _ Expresion _ %RBRACKET
    # d[0] LitPrim, d[1] _, d[2] %LBRACKET, d[3] _, d[4] Expr1, d[5] _, d[6] %RBRACKET, d[7] _, d[8] %LBRACKET, d[9] _, d[10] Expr2, d[11] _, d[12] %RBRACKET
    # {% (d) => nodo("AccesoArregloDoble", { matriz: d[0], indice1: d[4], indice2: d[10] }) %}
